...
def test_create_contract_invalid_property(auth_client):
...
def test_get_contract_by_key(auth_client):
    ...
    prop_res = auth_client.post("/properties", json={"property_name": "House 1", "owner_name": "Bob Owner", "address": "123 Green Rd", "room_count": 4})
    property_key = prop_res.json()["key"]
    ...
    response = auth_client.get(f"/contracts/{contract_key}")
    assert response.status_code == 200
    assert response.json()["rental_deposit"] == 2000.00

def test_update_contract(auth_client):
    tenant_res = auth_client.post("/tenants", json={"name": "Tenant One", "document_number": "222.333.444-55"})
    tenant_key = tenant_res.json()["key"]

    prop_res = auth_client.post("/properties", json={"property_name": "Studio X", "owner_name": "Owner One", "address": "456 Central St", "room_count": 1})
    property_key = prop_res.json()["key"]
