...
def test_get_property_by_key(client):
    payload = {
    ...
    create_res = client.post("/properties", json=payload)
    property_key = create_res.json()["key"]

    response = client.get(f"/properties/{property_key}")
    assert response.status_code == 200
    assert response.json()["owner_name"] == "Jane Doe"

def test_get_property_not_found(client):
    response = client.get("/properties/invalid-uuid-key")
...
def test_update_property(client):
    ...
    update_res = client.put(f"/properties/{property_key}", json=update_payload)
...
def test_delete_property(client):
    ...
    delete_res = client.delete(f"/properties/{property_key}")
    assert delete_res.status_code == 204

    get_res = client.get(f"/properties/{property_key}")
